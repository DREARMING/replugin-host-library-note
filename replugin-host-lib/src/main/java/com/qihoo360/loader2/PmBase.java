...
class PmBase {
    ...
    void init() {
        ...
        if (HostConfigHelper.PERSISTENT_ENABLE) {
            ...
            /**
             * Important:
             * 1. 这里必须要清楚，主进程即Application刚打开时，这里的IPC接口返回的绝对是false，然后直接调用initForClient
             * 2. initForClient 会通过 ContentProvider（ProcessPitProviderPersist.java）去开启 :GuardService 子进程（常驻进程），
             *    并且返回 PmHostSvc 的 IBinder 对象，这个时候 PmHostSvc 已经在常驻进程中了
             *
             */
            if (IPC.isPersistentProcess()) {
                ...
                /**
                 * 插件管理进程的初始化
                 *  1. 初始化 PluginManagerServer , PluginManagerProxy.load() 将会调用 PluginManagerService
                 *     的load 方法去加载 app_p_a 下的p.l 文件的 json数据，该文件记录了框架加载和下载的插件PluginInfo 列表，
                 *     可以看到，插件的管理都交由 PluginManagerService 去管理，保证全局唯一性
                 */
                initForServer();
            } else {
                // 连接到Server
...
//        }
        ...
        if (!IPC.isPersistentProcess()) {
            ...
            //这个广播将会在 常驻进程 的 PmHostSvc 更新完插件信息后，通过IPC跨进程把广播发过来
    ...
    final void newPluginFound(PluginInfo info, boolean persistNeedRestart) {
        ...
        //需要记住，LocalBroadcastManager 只能在本进程传播广播，不会跨进程。
        // 通知本进程：通知给外部使用者
